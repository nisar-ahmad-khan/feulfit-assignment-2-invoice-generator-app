...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference("Option.count") do
      post options_url, params: { option: { name: @option.name, option_group_id: @option.option_group_id, price: @option.price } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { name: @option.name, option_group_id: @option.option_group_id, price: @option.price } }
